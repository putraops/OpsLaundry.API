...
type Product struct {
	...
	ProductCategoryId string `gorm:"type:varchar(50);null" json:"product_category_id"`
	Description       string `gorm:"type:text" json:"description"`

	ProductCategory ProductCategory `gorm:"foreignkey:ProductCategoryId;references:id;constraint:onUpdate:CASCADE,onDelete:CASCADE" json:"-"`
...
func (r *Product) BeforeCreate(db *gorm.DB) (err error) {
...
func (r *Product) BeforeSave(db *gorm.DB) (err error) {
	user, err := utils.GetUserIdentity(db.Statement.Context)
	if err != nil {
		return err
	}
...
func (r *Product) BeforeDelete(db *gorm.DB) (err error) {
	user, err := utils.GetUserIdentity(db.Statement.Context)
	if err != nil {
		return err
	}
